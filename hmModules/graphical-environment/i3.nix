  ...
  in {
    ...
    config = {
      ...
      startup = [
        {
          command =
            "${pkgs.pasystray}/bin/pasystray --volume-max=100 --volume-inc=1 --notify=all";
          notification = false;
        }
        {
      ...
      modes = let systemctl = "${pkgs.systemd}/bin/systemctl";
      in lib.mkOptionDefault {

        ${mode_system} = {
          l = "exec --no-startup-id ${i3lock}, mode default";
          s =
            "exec --no-startup-id ${i3lock}; exec --no-startup-id ${systemctl} suspend, mode default";
          h =
            "exec --no-startup-id ${i3lock}; exec --no-startup-id ${systemctl} hibernate, mode default";
          u =
            "exec --no-startup-id ${pkgs.lightdm}/bin/dm-tool switch-to-greeter, mode default";
